...
### 🤖 Lobber (The Passer)
*   **Primary Reward**: `rew_passing` (+150 per ball passed) OR `reward/rew_stashing` (+ points for every ball sent deep).
*   **Station Goal**: Staying near the **Neutral Zone**.
*   **TensorBoard Tips**:
...
## 🏗️ Manager Training Glossary (Hierarchical RL)

When training the **Manager Agent**, time works differently because the Manager makes decisions at **1Hz** (Simulated Second) rather than 60Hz.

### ⏱️ Time & Frequency
*   **Timestep**: Represents **1 Second** of game time in the Manager's brain.
*   **Physics Frame**: Runs at 60Hz. The Manager waits for 60 physics frames between every decision.
*   **Worker Interval**: We optimize performance by only updating the Sub-Agents (Workers) every 6 physics frames (10Hz).

### ⚙️ Config Parameters
*   **`n_steps`**: The number of **Seconds** each parallel environment runs before sending data to the GPU.
    *   *Example*: `n_steps: 128` means the agent plays for ~2 Mini-Matches (128 seconds) before learning.
    *   *Total Batch Size*: `128 steps * 24 envs = 3,072` experiences per update.
*   **`eval_freq`**: How often we pause to run specific Test Matches.
    *   *Example*: `eval_freq: 1000` means "Every 1000 seconds of gameplay *per environment*".
    *   With 24 environments running at ~40x speed, this happens roughly every **10 Minutes**.
*   **`mini_match`**: A special training mode that shortens the game to 65 seconds to speed up learning.
    *   **0-10s**: Auto (Red Scores).
    *   **10-15s**: Transition (Both Score).
    *   **15-40s**: Stage 1 (Loser Scores).
    *   **40-65s**: Stage 2 (Winner Scores).
    *   **10-15s**: Transition (Both Score).
    *   **15-40s**: Stage 1 (Loser Scores).
    *   **40-65s**: Stage 2 (Winner Scores).
